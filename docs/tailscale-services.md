...
or CLI on the host** (the Tailscale package is still what you'd use for
cluster peering — the two are independent).
...
- This package running in **privileged mode** (the connector deploys via the
  `exec` driver, which needs a root Nomad client — see the README).
...
allocations don't rejoin as new devices. Create
`/var/packages/nomad/etc/conf.d/tailscale-connector.hcl`:

```hcl
client {
  host_volume "tailscale-connector-state" {
    path = "/var/packages/nomad/var/tailscale-connector"
  }
}
```

Then create the directory and restart Nomad:

```sh
sudo mkdir -p /var/packages/nomad/var/tailscale-connector
sudo synopkg restart nomad
```

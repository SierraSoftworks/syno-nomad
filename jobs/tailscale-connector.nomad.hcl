...
#
# Requires privileged mode (the exec driver needs a root Nomad client) and a
...
job "tailscale-connector" {
  ...
  group "connector" {
    ...
    task "connector" {
      ...
      artifact {
        # ${attr.cpu.arch} resolves to amd64 on x86_64 models and arm64 on
        # aarch64 models. Pin the version (and ideally a checksum) you intend
        # to run; releases are tagged connector-v<version>.
        source = "https://github.com/SierraSoftworks/syno-nomad/releases/download/connector-v0.1.0/nomad-tailscale-connector_0.1.0_linux_${attr.cpu.arch}.tar.gz"
      }
